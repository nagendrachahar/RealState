...
export class AccountLedgerComponent implements OnInit {
  ...
  LedgerList = [];
    ...
    if (Id != 0) {
      ...
      this.service.GetOneAccountLedger(Id)
        .subscribe((data) => {
          console.log(data);
          if (Id == 0) {
            this.SaveBtn = "Save";
          ...
          this.AccountLedgerform.controls['LedgerId'].setValue(data[0].LedgerId);
          this.AccountLedgerform.controls['GroupId'].setValue(data[0].GroupId);
          this.AccountLedgerform.controls['LedgerName'].setValue(data[0].LedgerName);
    ...
    public save() {
      this.service.SaveAccountLedger(this.AccountLedgerform.value)
            .subscribe((data) => {
  ...
  public Delete(LedgerId: number) {
    ...
    yesButton.addEventListener('click', () => {
      this.service.DeleteAccountLedger(LedgerId)
        .subscribe((data) => {
          this.Message = data[0].Message;
          if (data[0].MessageType == 1) {
  ...
  get LedgerId() { return this.AccountLedgerform.get('LedgerId');}
  get GroupId() { return this.AccountLedgerform.get('GroupId'); }
  get LedgerName() { return this.AccountLedgerform.get('LedgerName'); }
